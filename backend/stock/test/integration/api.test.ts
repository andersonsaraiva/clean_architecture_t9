...
test('Deve testar a api de estoque', async function () {
  ...
  const response1 = await axios.post('http://localhost:3003/calculateStock', {
    idProduct: 1
  });
  ...
  const response2 = await axios.post('http://localhost:3003/calculateStock', {
    idProduct: 1
  });
  ...
  const response3 = await axios.post('http://localhost:3003/calculateStock', {
    idProduct: 1
  });
